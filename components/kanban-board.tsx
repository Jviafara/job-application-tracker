...
function DroppableCollumn({
  ...
  return (
    <Card className='min-w-75 shrink-0 shadow-md p-0'>
      <CardHeader className={`${config.color} text-white rounded-t-lg pb-3 pt-3`}>
        <div className='flex items-center justify-between'>
          ...
          <DropdownMenu>
            ...
            >
              <DropdownMenuItem
                className='text-destructive pr-4'
      ...
      <CardContent
        ref={setNodeRef}
        className={`space-y-2 bg-gray-50 min-h-100 rounded-b-lg ${isOver ? ' ring-2 ring-blue-500' : ''}`}
      >
        <SortableContext
...
const KanbanBoard = ({ board }: KanbanBoardProps) => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    //Check if dropped in column or a different one
    ...
    if (targetColumn) {
      ...
      const targetIndexInFilterd = allJobsInTargetFiltered.findIndex(j => j._id === overId)

      if (targetIndexInOriginal !== -1) {
        if (sourceColumn._id === targetColumnId) {
          if (sourceIndex < targetIndexInOriginal) {
            newOrder = targetIndexInFilterd + 1
          } else {
            newOrder = targetIndexInFilterd
          }
        } else {
          newOrder = targetIndexInFilterd
        }
